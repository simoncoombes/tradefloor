...
- **Quotes around the model price.** The maker quotes around the last print, so the traded price can lag the model price by tens of basis points. A new dial centres the book on the model price every tick.
