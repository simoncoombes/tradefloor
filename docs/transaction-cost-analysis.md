...
build against the first name of `Universe.random(20, seed=7)` — ADV 9,713
shares — over one six-step day, the configuration the test suite pins.
...
shares — half that name's ADV, sim seed 2026 — filled 483, and requests of
...
comes back empty: order flow consumes no RNG draws — the per-domain stream
split exists for exactly this — so an untraded name saw byte-identical noise
...
measurement. When the untouched names must be byte-exact — isolating one
name's impact, or auditing the subtraction — pin VIX in both worlds:
