...
concentration (`top_k`), an exposure (`gross`) and a participation cap. The
spec is that grammar as JSON:
...
from each and merging, and the spec is unambiguous about which. Each component
...
agent is invariant under any positive scaling of the weight vector, so weights
...
nothing. And `to_json` always writes the full canonical form, so a reader of
...
those tests fails, the canonical form has changed, and with it the meaning
of every published fingerprint. The failure message says what that
...
- `"step"` is the default, and what every shipped baseline does: re-decide at
...
- `"daily"` re-decides on the first step of each day, whatever
...
refuses a cadence outright, because it trades once, so a cadence on it describes
...
`strategy_fingerprint` is empty, which is the honest reading, because such a result
