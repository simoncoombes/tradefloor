...
from check import SpecError, check  # noqa: E402
...
def committed(mech: Mechanism) -> str | None:
    ...
    if j < 0:
        raise EmitError(f"{mech.target['file']} has a begin marker for "
                        f"{mech.name} and no end marker")
    return text[i:j + len(end)]


def write(mech: Mechanism) -> str:
    """Replace the function body in the target with the generated one.

    On the first write, the whole existing body of the target function is
    replaced; after that, only what lies between the markers.
    """
    path = os.path.join(ROOT, mech.target["file"])
    with open(path, "rb") as f:
        text = f.read().decode("utf-8").replace("\r\n", "\n")
...
def main(argv=None) -> int:
    ...
    for mech in shipped():
        ...
        if a.check:
            have = committed(mech)
            want = Emitter(mech).function_body()
            if have != want:
                drift += 1
                print(f"  {mech.target['file']}: the committed "
                      f"{mech.target['function']} differs from the generated one")
    if a.check:
