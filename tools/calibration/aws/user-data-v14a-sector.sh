...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14a-sector
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14a.json" \
...
# pt-v14 must beat pt-v12 on the 504-day panel while regressing NEITHER
# crisis co-movement (which disqualified r15-70) NOR the crisis lever
# (which disqualified r15-86). Round 46 showed why that is hard: of forty
# neighbours, fourteen cleared the co-movement CEILING on block 1101 and
# every single one of them broke the FLOOR on another block. The real band
# is 0.063 wide and pt-v12's across-block spread is already 0.045, so the
# level and the spread have to move together or not at all.
#
# The partial-effects fit over those forty points says the sector block is
# the one place where all three signs favour us: sector_factor_sigma UP
# lowers the level, narrows the spread and lowers lever error, while
# sector_loading UP does the reverse. Mechanically that is more sector
# volatility carried on less common loading -- names decorrelate ACROSS
# sectors rather than uniformly, which is also the shape the cross-
# sectional dispersion gap has been asking for.
#
# Round 46 could not have found this: it swept sector_factor_sigma over
# 0.00967..0.01046, a span of 0.0008, well inside its own seed noise.
# This run sweeps 0.007..0.021 and sector_loading 0.42..0.63.
#
# Four blocks, chosen to bracket the whole co-movement distribution rather
# than to be survivable: 401 sits nearest the floor at 0.692, 201 mid at
# 0.717, and 1101 and 1301 are the two worst ceiling breaches at 0.737 and
# 0.747. Holding all four means holding the spread, not just the level.
