...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd25-confirm-lh
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd25.json" \
...
# Round 24's survey found six vectors clearing all five constraints on
# block 201, where all three references FAIL it: pt-v12 on the 504-day
# panel, d3-f10 on the 504-day panel, and p970 on the crisis lever at 5.81.
#
# The structure in the survivors is the point. They order almost exactly by
# persistence and the driven ratio tracks it -- 0.9784 gives 1.448 and
# 0.9694 gives 1.545 -- and EVERY ONE carries a non-zero funded
# `jump_vix_coupling`, between 0.137 and 0.349. None has zero. p970 has
# zero.
#
# That is the interaction hand search could not reach. Round 21 measured
# persistence costing the lever; round 15 measured funded jump coupling
# raising it. The survivors pair them, so persistence buys the driven
# window and spends the lever while the coupling buys the lever back. The
# pair was never tried by hand because round 15 tested coupling against
# `idio_sigma_scale`, not against persistence.
#
# It also reaches settings that FAIL alone: lh03 runs persistence 0.9784,
# and 0.980 on its own was 13/14 at the certified horizon with co-movement
# through the floor.
#
# This is round 13's lesson applied in advance. That screen's winner did not
# survive the four blocks it had not seen, so the top four here run on all
# four of theirs, with all three references beside them.
for BLOCK in "" "--seed-start 301" "--seed-start 401" "--seed-start 501"; do
