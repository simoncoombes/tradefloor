...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd37-confirm-n19
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd37.json" \
...
# Round 36 screened the last twenty untouched dials and the prediction
# written into it held. `market_vol_vix_anchor` correlates -0.97 with the
# driven ratio -- the strongest single effect found in thirty-six rounds --
# and the mechanism is plain: the market variance target scales with
# (VIX/anchor)^2, so moving the anchor rescales the WHOLE curve rather than
# a point on it. Round 17 called half the driven defect a flat gain error;
# this is the dial that addresses exactly that half.
#
# It ships at 15.0 and has never been varied. Round 19 tested the ceiling
# and the coupling as causes of the curve's shape and never tested the
# third term in the same expression.
#
# The cost is the familiar one: -0.80 against crisis co-movement. Raising
# the anchor lowers the market factor's share, which is the constraint that
# has bound every candidate since round 21.
#
# Three of forty-five clear both screen blocks. n19 reads 1.237 and 1.397
# against c28's 1.292 and 1.465, with co-movement 0.683 against a floor of
# 0.664. This confirms them on the three blocks neither screen used, which
# is the step that has killed every previous screen winner.
