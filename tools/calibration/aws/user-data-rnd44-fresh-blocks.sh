...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd44-fresh-blocks
...
# r15-70 is qualified on every axis. What it has not had is a test of the
# thing its own history makes most suspect.
#
# It was selected through four stages: a 48-point cube screened on one
# block, six survivors, a refinement, a twenty-dial screen on two blocks,
# and a parsimony reduction. Blocks 101-130, 201-230, 301-330, 401-430 and
# 501-530 were all used somewhere in that chain -- as screens, as
# confirmations, or as both. Cumulative selection over five blocks is not
# the same as five independent measurements, and no amount of internal
# checking fixes it.
#
# So: two blocks that have never been touched by anything in this
# programme, 601-630 and 701-730. Nothing has been screened on them,
# confirmed on them, or refined against them. If r15-70's advantage over
# pt-v12 is real it appears here at full size; if part of it was selection,
# it shrinks.
#
# This is worth more than another round of optimisation. The candidate is
# already better than everything else measured; the open question is how
# much of that survives contact with data the search never saw.
for BLOCK in "--seed-start 601" "--seed-start 701"; do
...
if [ -f /home/ec2-user/out/gates-601.json ] && [ -f /home/ec2-user/out/gates-701.json ]; then
