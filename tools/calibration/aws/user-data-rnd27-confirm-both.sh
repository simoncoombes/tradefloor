...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd27-confirm-both
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd27.json" \
...
# Re-screening the same forty-eight vectors on block 401, where crisis
# co-movement is tightest, gives a DIFFERENT set of survivors. Only two
# vectors clear both screens:
#
#   clears on 201 (easiest co-movement)  lh02 lh03 lh07 lh16 lh32 lh35
#   clears on 401 (tightest)             lh02 lh13 lh16 lh28  + v12, d3-f10
#   clears on BOTH                       lh02 lh16
#
# Round 25 confirmed lh03, lh07, lh35 and lh32 -- the top four by driven
# ratio on block 201 -- and all four failed. NONE of them clears on 401.
# lh02 and lh16 clear on both and were ranked fifth and sixth by that
# objective, so they were dropped from the confirmation set.
#
# That is a second selection error on top of the screen-block one, and it
# is the same mistake in a different place: ranking by the OBJECTIVE
# picked vectors that maximise the thing being improved, where ranking by
# ROBUSTNESS -- clearing on more than one block -- would have picked the
# two that survive. The objective is what the search is for; it is not what
# the shortlist should be sorted by.
#
# So this confirms the two that clear both, plus the two that clear only the
# tight screen, on the three blocks neither screen used: train, 301 and 501.
# pt-v12 and d3-f10 ride along, and d3-f10 is the incumbent to beat -- it
# clears three of four unseen blocks and has never regressed anything.
