...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/cma-g1
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-cma-g1.json" \
...
# CMA-ES generation 1, centred on s23, in the eight dials the posterior
# says are identified once the crisis instruments are included.
#
# This is the first generation this programme has run that was not a Latin
# hypercube. Random fill samples a thin shell in fifteen dimensions, which
# is why every previous round needed a neighbourhood sweep afterwards to do
# the real work; CMA-ES adapts the step size AND the covariance, so it
# learns the correlations between dials instead of rediscovering them.
#
# Two of the eighteen proposals were repaired before launch: the box bounds
# each dial independently and cannot express GARCH stationarity, so
# market_vol_alpha + market_vol_beta is scaled back under 0.995 with the
# ratio the search chose preserved. The repaired point is what is measured
# AND what is stored as pending, so the update learns from what actually ran.
#
# Same four blocks s23 was searched on, so the generation is directly
# comparable to its 4/4 without re-running the incumbent's other nine.
