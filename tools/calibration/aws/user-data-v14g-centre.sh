...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14g-centre
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14f.json" \
...
# The nine-block confirmation said something more useful than pass or fail.
# Crisis co-movement's real band is 0.664-0.727, centre 0.6955, and the
# candidates are not scattered around it -- they are on opposite SIDES:
#
#   pt-v12  +0.026 off centre, fails through the CEILING
#   s23     -0.021 off centre, fails through the FLOOR
#   lh16    +0.003 off centre, out on one block of nine
#
# s23's correction is about twice what was needed, and its fourteen
# neighbours inherit that: seven of them fall through the floor. It is not
# on a knife edge, it is on the wrong side of centre.
#
# So: sixteen vectors, three ways of getting to the middle.
#
#  i15..i90  the line from s23 to lh16. Co-movement should cross centre near
#            t=0.85, but the 504 rate need not move linearly along it, which
#            is the reason to sample the line rather than solve for a point.
#  sf86..97  s23 with ONLY sector_factor_sigma walked back. It measured rho
#            -0.81 against co-movement over a sweep twenty-six times wider
#            than any before it, so lowering it should raise co-movement
#            without disturbing the jump balance the lever depends on.
#  gc60,75   the same via garch_vix_coupling, rho -0.89 on the hard blocks.
#
# The single-dial routes matter because of what CMA-ES generation 1 just
# showed: searching eight of the fifteen dials while freezing the other
# seven wrecked the lever on all eighteen candidates (0.05 to 0.28 against a
# 0.05 tolerance). This vector is a joint solution. If a one-dial walk-back
# works it is because that dial moves co-movement without breaking the
# balance -- and that has to be demonstrated, not assumed.
#
# Seven blocks spanning pt-v12's whole co-movement range, so a candidate
# near the centre is tested against BOTH edges: 1401 sits lowest at 0.692
# and 1301 highest at 0.747.
for BLOCK in "--seed-start 701" "--seed-start 201" "--seed-start 801" \
  ...
  # Trap 19: upload each block as it lands. Seven blocks is over an hour of
  # spot exposure and a reclamation at block six must not discard five.
