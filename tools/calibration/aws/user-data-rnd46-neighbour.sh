...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd46-neighbour
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd46.json" \
...
# The era-boundary checklist's entry bar has three items. Two are done:
# the thirty-seed gate on every axis, and section 8 with the incumbent as
# its control. The third is "a neighbour sweep showing it is a REGION
# rather than a lucky point", and r15-70 has never had one -- round 29
# swept around lh02 and round 41 varied a single dial.
#
# So: forty vectors, every one of r15-70's fifteen dials moved
# independently by up to +/-4%. If the neighbourhood holds the panel, the
# candidate is a basin and the exact coefficients are not load-bearing. If
# most neighbours fail, r15-70 is a spike and shipping it would be shipping
# a coincidence.
#
# Three blocks: 201 and 401, which the search used, and 1101, which is the
# hardest block measured -- pt-v12 reads 10 of 14 at 504 there and misses
# the certified panel too. A basin should survive the hard block.
