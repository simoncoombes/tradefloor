...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/u63confirm
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-u63confirm.json" \
...
# The thirteen-block confirmation of the untouched-dial screen (round 93).
#
# What the screen ranked, thirteen blocks decide: sector dispersion
# bracketed around the cell that broke the round-90 frontier, the volume
# response bracketed around the cell that lifted volume_abs_return_corr
# off its floor, the combined fold candidate, the stress memory retried
# at a weight the panel might tolerate, the regime wire jointly with the
# stress weight that gates it, the day-fear cell with the clamp opened to
# the FRED-measured scale, and the realised-vol feedback at a sane blend
# weight (the screen's 2.0 was outside the dial's [0,1] semantics).
#
# Anything confirmed here folds INTO pt-v15 before it ships (decision
# 2026-08-29). The preset is registered but unreleased, so its definition
# is still free.
#
