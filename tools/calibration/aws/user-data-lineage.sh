...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/lineage
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-lineage.json" \
...
# lineage: every preset from pt-v12 to pt-v16, one build, one run.
#
# The comparative record has been stitched across eras; this measures the
# whole line on the current build so the pt-v16 release can state its
# ancestry from a single certified corpus, and pt-v12/pt-v13 finally get
# thirteen-block numbers on the instrument set that judges their
# successors.
#
