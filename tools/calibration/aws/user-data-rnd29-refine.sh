...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd29-refine
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd29.json" \
...
# lh02 is a RANDOM point in a 48-sample cube over six dimensions, so the
# local optimum is almost certainly somewhere else nearby. This refines it:
# thirty-six samples in a narrow box around its coordinates, which is a
# local search justified by the global one rather than another guess.
#
#   persistence  0.9665 - 0.9750   lh02 at 0.9707
#   jump d          1.4 - 2.9      lh02 at 2.07
#   jump f         0.80 - 0.96     lh02 at 0.87
#   idio_sigma     0.50 - 0.60     lh02 at 0.569
#   garch_vix      0.00 - 0.16     lh02 at 0.027
#   jump_vix       0.18 - 0.34     lh02 at 0.263
#
# BOTH SCREEN BLOCKS, IN ONE RUN, and that is the point of this round's
# design rather than an afterthought. Rounds 24 to 27 cost two wasted
# confirmations to two selection errors, and both are fixed here:
#
#   Round 24 screened on 201 alone, where crisis co-movement is nearly
#   loosest, so it selected vectors that clear a FLOOR where the floor is
#   easiest. They failed everywhere else.
#
#   Round 25 then confirmed the top four BY DRIVEN RATIO, which selects
#   whatever sits hardest against the binding constraint. The two vectors
#   that clear both screens ranked fifth and sixth and were dropped.
#
# So: screen on 401 where co-movement is tightest AND 201 where it is
# loosest, and rank what clears BOTH. The objective is what a search is
# for; it is not what a shortlist is sorted by.
