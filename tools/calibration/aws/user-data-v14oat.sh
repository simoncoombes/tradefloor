...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14oat
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14oat.json" \
...
# pt-v14 is registered and is NOT the default, for one reason: its crisis
# lever is a sharp optimum rather than a basin. Six neighbours at plus or
# minus three percent across all fifteen dials hold the 504 panel and crisis
# co-movement, but only two keep the lever inside its five percent
# tolerance on every block.
#
# Six points in fifteen dimensions cannot say WHICH dial is responsible.
# This does: every dial perturbed ONE AT A TIME, at -6%, -3%, +3% and +6%,
# with all fourteen others held at pt-v14's values. Fifty-nine probes.
#
# If the fragility concentrates in one or two dials, the fix is to constrain
# those and re-search the rest -- and pt-v14 becomes a defensible default.
# If it is spread evenly across all fifteen, the vector is a genuine point
# optimum and the answer is a different search objective: optimise the
# lever's ROBUSTNESS over a neighbourhood rather than its value at a point.
# Either way the next move follows from the answer rather than from a guess.
#
# Three blocks, not seven: the corpus says the crisis lever is 95%
# determined by the parameter vector and only 5% by the seed block, so this
# is the one measurement in the programme that does not need many blocks.
for BLOCK in "--seed-start 201" "--seed-start 401" "--seed-start 1101"; do
...
if [ -f /home/ec2-user/out/gates-201.json ] && [ -f /home/ec2-user/out/gates-1101.json ]; then
