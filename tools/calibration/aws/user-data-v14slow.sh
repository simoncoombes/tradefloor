...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14slow
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14slow.json" \
...
# The two-component variance mixture, enabled for the first time.
#
# An audit of the corpus found 63 of the model's 120 numeric dials have
# never been varied in 732 measured vectors. The largest untouched thing is
# a whole mechanism: market_vol_slow_weight, _persistence, _gain and
# _vix_damp are all zero on every shipped preset, so the market factor's
# variance runs on ONE timescale.
#
# Its own docstring says why it exists: "real volatility memory decays
# hyperbolically and a single exponential cannot imitate that past about a
# year", and calls that gap 3. It has never appeared in a calibration
# search.
#
# Why it is the right thing to try now. Round 79 measured the binding
# limit: crisis co-movement's across-block RANGE is bought with 504-day
# panel blocks, mediated by GARCH persistence, because one variance
# timescale does two jobs. Round 80's damp failed trying to break that link
# downstream, at the crisis injection. This breaks it upstream, where the
# code comment says it belongs -- "the fast component chases VIX, the slow
# one carries the autonomous level and the long memory".
#
# market_vol_slow_vix_damp is the dial that makes the two components do
# DIFFERENT jobs: at 0 both revert to the same VIX-scaled target and the
# mixture is only two decay rates; above 0 the slow target is less
# VIX-coupled. Its docstring records it as a measured negative -- but
# against restoring a scenario TRANSIENT, which is not this target. The
# same mistake pattern as crisis_blend_ramp, which no held-VIX measurement
# could ever have seen.
#
# Three weights x three damps, plus two slow timescales at the middle
# weight. Four blocks spanning the co-movement distribution so the RANGE is
