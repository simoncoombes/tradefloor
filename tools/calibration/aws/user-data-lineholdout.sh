...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/lineholdout
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-lineholdout.json" \
...
# holdout: the full house out of sample, plus margin hardening.
#
# pt-v16's thin margins were selected on the thirteen blocks the search
# used; these thirteen (1501-2701) were touched by no search this era.
# The frozen preset runs beside four near-neighbours: if a neighbour
# holds the same card with fatter margins out of sample, it is the
# better composition and the fold moves to it; if pt-v16 alone holds,
# the release claim is armoured; if the house falls out of sample, we
# learned it for a dollar instead of in production.
#
...
for BLOCK in 1501 1601 1701 1801 1901 2001 2101 2201 2301 2401 2501 2601 2701; do
...
if [ -f /home/ec2-user/out/gates-2701.json ] && [ -f /home/ec2-user/out/gates-1501.json ]; then
