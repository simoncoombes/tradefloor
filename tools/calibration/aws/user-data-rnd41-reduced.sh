...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd41-reduced
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd41.json" \
...
# Round 40 named the compensator. Sweeping the nineteen against the CRISIS
# LEVER, which round 36's screening analysis never did:
#
#   endogenous_news_sigma   -0.77 vs lever, +0.71 vs driven
#   crisis_blend_cap        +0.60 vs lever, +0.69 vs driven
#   garch_ceiling_multiple  +0.40 vs lever, -0.19 vs driven
#
# `endogenous_news_sigma` carries it, and it is the second dial in this
# programme that helps two things at once: lowering it RAISES the lever, so
# it pays the anchor's bill, and it LOWERS the driven ratio at the same
# time. It is the endogenous news channel and not the caller-supplied one
# section 85 measured at zero -- `endogenous_news_intensity` ships at 0.05
# and its sigma at 0.03, and both are live in a bare run.
#
# So the reduction is explicit rather than sampled:
#
#   c28                 13 dials
#   + market_vol_vix_anchor      14, rescales the whole VIX curve
#   + endogenous_news_sigma      15, pays the anchor's lever cost
#
# Three values of the sigma, against n19's thirty-three and against
# c28+anchor which fails the lever. If fifteen dials with a named reason for
# each land where thirty-three do, the thirty-three-dial vector is retired
# and the candidate is explicable.
