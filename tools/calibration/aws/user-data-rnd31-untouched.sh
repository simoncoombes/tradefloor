...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd31-untouched
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd31.json" \
...
# Thirty rounds have moved 14 of 87 settable dials. Twenty ship at 0.0 and
# are inert by construction, which leaves FIFTY-THREE live dials this
# programme has never touched. The priority order's top step is "score a
# dial that already exists", and it is nowhere near finished.
#
# Seventeen of them here, each swept +/-20% of its shipped value: the
# per-name GARCH triple, the VIX process itself, the crisis blend, the crash
# amplifier, the crowd and momentum terms, mispricing decay, sector loading,
# informed flow and endogenous news intensity.
#
# The GARCH triple is reparameterised as persistence plus fractions, the way
# `atlas_survey` does it, so alpha + beta + gamma/2 stays stationary by
# construction rather than by luck.
#
# `jacobian.py` exists and measures exactly this -- one dial at a time,
# central differences. It is not used here for the reason round 24 exists:
# single-dial derivatives miss interactions, and this programme has now
# found three shared budgets that only a joint design could see. A Latin
# hypercube gives the same screening information and gives it in
# combination.
#
# The expectation is that most of these do nothing measurable -- they are
# untouched because earlier work had no reason to touch them, not because
# nobody looked. What this can find is a dial with leverage that no
# single-dial pass would have flagged, which is the same class of result as
# round 24's persistence-and-coupling pair.
