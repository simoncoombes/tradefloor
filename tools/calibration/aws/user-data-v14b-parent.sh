...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14b-parent
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14b.json" \
...
# The corpus has been holding a lead nobody looked at. Pooling all 1201
# candidate-block panels this programme has run and ranking on BLOCK-AVERAGED
# outcomes -- not one block, which is 44% seed noise -- four vectors beat
# pt-v12 on the 504 panel while LOWERING crisis co-movement and holding the
# lever. Two of them, n19 and lh02, read a perfect 504 panel with
# co-movement in range on every block they were measured on.
#
# The catch, and the reason this run exists: they were measured on five
# blocks -- 201, 301, 401, 501 and train -- and those are the blocks the
# SEARCH used. None of them is a block where pt-v12's co-movement actually
# fails. n19 has never been run on 1001, 1101, 1201 or 1301.
#
# n19 is also the 33-dial parent that r15-70 and r15-86 were reduced FROM,
# and both reductions regress something on the hard blocks: r15-70 puts
# co-movement outside on five blocks of thirteen, r15-86 gives up the
# crisis lever. If the parent holds where the reductions do not, the
# reduction cost the property, and that is worth knowing before pt-v14 is
# built on top of a reduced vector.
#
# So: the four clean vectors, plus pt-v12, r15-70 and r15-86 as controls,
# on the four blocks nobody has run them on.
for BLOCK in "--seed-start 1001" "--seed-start 1101" "--seed-start 1201" "--seed-start 1301"; do
...
if [ -f /home/ec2-user/out/gates-1001.json ] && [ -f /home/ec2-user/out/gates-1301.json ]; then
