...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd8-roster
...
# The roster-concentration gap says a concentrated roster does not inherit
# the envelope past one year, and that the three counts it carries -- 9, 8
# and 7 -- are from the pt-v3 era, a ten-statistic panel and six seeds. It
# has been two eras and four statistics out of date since.
#
# roster_shapes.py re-measures it at thirty seeds on the fourteen-statistic
# panel at both horizons. A single-seed smoke run already shows what the gap
# describes: sp500_like reads 9/14 at 504 days against 14/14 at 252.
#
# The candidates run beside it because a preset that is better on the
# certified roster and worse on a concentrated one is not better.
test -f tools/calibration/roster_shapes.py
for spec in "v12:::" \
            "fund-047:pt-v12:idio_sigma_scale=0.47" \
            "d3-f08:pt-v12:jump_intensity_idio=0.006058633,jump_sigma_idio=0.09662265" \
            "d3-f08-f47:pt-v12:jump_intensity_idio=0.006058633,jump_sigma_idio=0.09662265,idio_sigma_scale=0.47"; do
  name="${spec%%:*}"; rest="${spec#*:}"; base="${rest%%:*}"; ov="${rest#*:}"
  if [ -z "$base" ]; then
    python tools/calibration/roster_shapes.py --seeds 30 \
      --workers WORKERS_PLACEHOLDER \
      --out "/home/ec2-user/out/roster-${name}.json" \
...
if [ -f /home/ec2-user/out/roster-v12.json ] \
   && [ -f /home/ec2-user/out/roster-d3-f08-f47.json ]; then
