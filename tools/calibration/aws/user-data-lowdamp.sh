...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/lowdamp
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-lowdamp.json" \
...
# Below the damp that was bracketed, because the bracket was on the wrong side.
#
# Round 84 derived damp 0.452 from four blocks as the cell landing the crisis
# lever on the real 6.16, and the thirteen-block run says 0.374. The fit was
# not the problem: R-squared on the lever is 0.9994 over four damps. The four
# blocks were. That is the same lesson this record has written three times,
# arriving this time on its own arithmetic.
#
# What thirteen blocks also showed is that every cell measured prefers a LOWER
# damp on both instruments: the co-movement range grows with damp
# (0.0498, 0.0531, 0.0537, 0.0559 at 0.35 through 0.48) and blocks in range
# fall away (12, 10, 8, 6). 0.35 was the floor of the bracket, so the optimum
# was never enclosed.
#
# The lever sets the real floor. From `lever = 7.1285 - 2.5868 * damp`, the
# median stays inside the 5% tolerance for damp in [0.255, 0.493], and the
# range is minimised at the low end -- about 0.0454 extrapolated, against
# pt-v12's 0.0551 and a band width of 0.0630. So this run walks 0.26 to 0.374
# and repeats 0.35 as a control, because a sweep that cannot reproduce its own
# previous cell is measuring the harness rather than the model.
#
# d29w45 varies the one axis that has never moved at low damp: the slow
# component's WEIGHT, fixed at 0.35 in every cell measured so far.
