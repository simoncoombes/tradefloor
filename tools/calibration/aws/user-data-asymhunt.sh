...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/asymhunt
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-asymhunt.json" \
...
# asymhunt: the corr_asymmetry screen, on the blocks that fail it.
#
# Simon's objective is 13/13 full-house; the gap is corr_asymmetry at
# 701 and 1201, so this screen measures ON those blocks (plus 201 and
# 401, the crisis binders, so a winner cannot hide a crisis cost). The
# new down-transmission dial at three values, the four known small
# movers at gentler values than round 93 screened, and one combination.
#
...
for BLOCK in 701 1201 201 401; do
...
if [ -f /home/ec2-user/out/gates-401.json ] && [ -f /home/ec2-user/out/gates-701.json ]; then
