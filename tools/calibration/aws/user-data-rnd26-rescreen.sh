...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd26-rescreen
...
# Round 24 screened forty-eight vectors on block 201 and round 25 killed all
# four leaders on the blocks they had not seen. The cause is not sampling
# luck, which is what was flagged before the run; it is a systematic bias in
# the screen, which is worse.
#
# Crisis co-movement has a FLOOR at 0.664, and pt-v12 reads it at 0.696,
# 0.717, 0.728, 0.692 and 0.723 across the five blocks. Block 201 is 0.717,
# its second highest. Every survivor carries a funded `jump_vix_coupling`,
# which adds per-name variance in a crisis and therefore LOWERS co-movement
# by construction. So the screen selected vectors that sit just above the
# floor on the block with the second-highest base, and they fall through it
# on train and 401 where the base is 0.696 and 0.692.
#
# Block 201 was picked because it is the hardest block for the 504-day
# PANEL. Nobody checked whether it was representative for the constraint
# that would end up binding, and it is not.
#
# So: the same forty-eight vectors, unchanged, screened on 401 -- the block
# where crisis co-movement is TIGHTEST. Anything clearing there is clearing
# where the binding constraint is hardest rather than easiest, which is the
# only single-block screen that can be trusted for a floor.
#
# The direction is not in doubt. lh03 averages a driven ratio of 1.405
# across four blocks against pt-v12's 1.536, the largest movement this
# programme has measured, with the lever at 6.12 against a real 6.16. The
# question is only whether any of it is affordable.
...
if [ -f /home/ec2-user/out/gates-401.json ]; then
