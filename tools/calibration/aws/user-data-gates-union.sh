#!/bin/bash
# The union variant of the gates box (pt-v17 era): one box measures a
# LIST of seed blocks serially, uploading each block's gates the moment
# it exists (trap 19: a spot reclaim at block eight must not discard
# blocks one to seven). Three or four of these cover the 26-block union
# without asking any region for quota it does not have.
#
...
setsid nohup /home/ec2-user/stream.sh "$BUCKET" "__SHARD__" >/var/log/pretium-stream.log 2>&1 < /dev/null &
...
sed -i "s|BLOCKS_PLACEHOLDER|${BLOCKS}|" /home/ec2-user/run.sh
...
GOT=$(ls /home/ec2-user/out/gates-*.json 2>/dev/null | wc -l)
WANT=$(echo "$BLOCKS" | wc -w)
if [ "$GOT" -eq "$WANT" ]; then
  echo "CERTIFIED exit=$STATUS blocks=$GOT/$WANT shard=__SHARD__" > /tmp/DONE
else
  echo "FAILED exit=$STATUS blocks=$GOT/$WANT shard=__SHARD__" > /tmp/DONE
fi

aws s3 cp /var/log/pretium-run.log "$BUCKET/run-__SHARD__.log" || true
for f in /home/ec2-user/out/*; do aws s3 cp "$f" "$BUCKET/" || true; done
aws s3 cp /tmp/DONE "$BUCKET/DONE-__SHARD__" || true
