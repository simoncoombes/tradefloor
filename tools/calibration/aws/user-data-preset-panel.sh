...
# a minute on 190 workers; everything else is provisioning and the Rust
# build, which have measured at 12 and 10 minutes. 90 leaves room for a slow
# build without leaving a 192-core box running on a job that has hung.
shutdown -h +90

exec > >(tee /var/log/pretium-run.log) 2>&1
set -x

BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/preset-panel
# `dev`, not `main`: the tool this run needs was pushed to dev at 94ba605 and
# main is thirteen commits behind. Verified with `git show origin/dev:...`
# before launch, which is runbook trap 16.
BRANCH=dev
# 190, not 192. Two cores left for the streamer and the OS; a pool sized to
