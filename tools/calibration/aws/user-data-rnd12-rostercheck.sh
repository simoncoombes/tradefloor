...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd12-rostercheck
...
# d3-f10 beats pt-v12 on five axes, ties on five, and loses one: the
# defensive roster at 504 days, where cross-sectional correlation reads
# 0.4120 against a ceiling of 0.410. That is 0.002 in level on a statistic
# whose seed-to-seed spread is 0.0999, so it is two percent of one standard
# deviation and pt-v12 sits at 0.4063 on the same roster, 0.037 seed-sd from
# the same edge. Both are on the line.
#
# Two percent of a standard deviation from ONE thirty-seed block is not a
# measurement, it is an inference. This runs the same five rosters on three
# more blocks. If d3-f10 is over on one of four, it was a draw; if it is
# over on three or four while pt-v12 is not, it is real and d3-f10 costs a
# roster.
test -f tools/calibration/roster_shapes.py
for START in 201 301 401; do
  for spec in "v12:::" \
              "d3-f10:pt-v12:jump_intensity_idio=0.0060586328,jump_sigma_idio=0.10802741"; do
    name="${spec%%:*}"; rest="${spec#*:}"; base="${rest%%:*}"; ov="${rest#*:}"
    if [ -z "$base" ]; then
      python tools/calibration/roster_shapes.py --seeds 30 --seed-start "$START" \
        --workers WORKERS_PLACEHOLDER \
        --out "/home/ec2-user/out/roster-${name}-${START}.json" \
        2>&1 | tee "/home/ec2-user/out/roster-${name}-${START}.txt"
    else
      python tools/calibration/roster_shapes.py --seeds 30 --seed-start "$START" \
...
if [ -f /home/ec2-user/out/roster-v12-201.json ] \
   && [ -f /home/ec2-user/out/roster-d3-f10-401.json ]; then
