...
SAMPLES=3000
# The §80 surface: the two things pt-v10 still gets wrong, on its own base.
# First, volume. pt-v10 holds volume_change_acf1 at 252 days and is the only
# row of fourteen that misses at 504, where the band tightens to -0.29/-0.21
# and the model reads -0.3156. §73 measured that longer volume memory moves
# it the WRONG way and that a bigger innovation takes volume_abs_return_corr
# through its own one-year floor, so the question is whether the three volume
# parameters have an interior point that holds both rows at both horizons, or
# whether the shape of the mechanism forbids it. Three axes answer that:
# volume_persistence, volume_innovation_sigma, volume_variance_gain.
#
# Second, crises. The volatility lever reads 5.05x against a real 6.16x, and
# sector structure under a held VIX 45 reads +0.040 against a real +0.103,
# which is the half of the retired sector gap that did not close. Four axes:
# sector_vix_coupling and crisis_blend_cap/_ramp shape what a crisis does to
# co-movement, market_vol_ceiling_multiple caps how violent it can get.
#
# Seven axes, 3000 LHS points, 144000 tasks, ~214 core-hours, about 2.3 hours
# on 94 workers. Zero infeasible on the plan, which needed the ceiling axis
# bounded by its own hard cap of 50 first. Columns to read: the fourteen-stat
# panel at BOTH horizons, vol_lever, sector_ex_45 and corr_blend.
BASE=pt-v10
ONLY=volume_persistence,volume_innovation_sigma,volume_variance_gain,sector_vix_coupling,crisis_blend_cap,crisis_blend_ramp,market_vol_ceiling_multiple
