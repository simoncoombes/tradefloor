...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd30-wider
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd30.json" \
...
# Round 29 confirmed lh02 is the robust optimum of its NEIGHBOURHOOD. This
# asks whether the neighbourhood was the right place to look.
#
# Round 24's cube was bounded by what this programme had already measured,
# and every one of those bounds came from a SINGLE-DIAL experiment. Round 24
# then proved single-dial results do not compose -- that is the whole reason
# it exists. So the bounds are themselves suspect. "f below 0.80 loses
# cross-sectional correlation" was measured in round 6 with f moving alone,
# and lh02 runs f = 0.87 with three other dials moving; there is no reason
# that boundary sits in the same place in eight dimensions as it did in one.
#
# So: widen every bound, and add two axes never surveyed at all.
#
#   persistence  0.960 - 0.982   was 0.9631 - 0.980
#   jump d         1.0 - 4.0
#   jump f        0.65 - 1.05    was 0.80 - 1.00
#   idio_sigma    0.44 - 0.62    was 0.47 - 0.58
#   garch_vix     0.00 - 0.60    was 0.00 - 0.45
#   jump_vix      0.00 - 0.50    was 0.00 - 0.35
#   sector_vix    0.60 - 1.00    NEW -- ships at 1.0, never varied here
#   market_vol_vix 0.90 - 1.02   NEW -- ships at 0.954
#
# Both screen blocks in one run and ranked by clearing both, which is round
# 29's design and the one that worked.
#
# The honest expectation is that most of the widened region is worse: the
# bounds were not arbitrary, they came from measurements. What this can find
# is the case where a bound MOVED because something else moved with it, and
# that case is invisible to every round before 24.
