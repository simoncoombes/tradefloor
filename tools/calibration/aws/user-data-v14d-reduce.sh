...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14d-reduce
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14d.json" \
...
# How many of s23's fifteen dials does pt-v14 actually need?
#
# ABC-SMC over the corpus says the fourteen-statistic panel identifies
# almost nothing -- 20% of the explored box satisfies all fourteen bands --
# and that adding crisis co-movement and the crisis lever brings the
# identified set only to three dials of fifteen, with nine unidentified
# under everything measured. If that is right, most of s23's dials are one
# arbitrary point on a ridge and reverting them costs nothing.
#
# A ladder, ordered by posterior contraction: 3, 4, 5, 6, 7, 9, 11, 13
# dials, each reverting the rest to pt-v12. Where the ladder breaks is the
# number pt-v14 actually needs, and an era boundary is far easier to
# justify and to document at six dials than at fifteen.
#
# One deliberate departure from the contraction order: garch_vix_coupling
# is kept at rank four despite SBI reading it as unidentified. Round 49
# measured its rank correlation with hard-block co-movement at -0.89. The
# emulator cannot see the effect because no vector in the corpus except
# lh16 varies the dial -- that is a sampling limit, not evidence of
# inertness, and a dial with a measured effect is not dropped on an
# emulator's say-so.
#
# `tail8` is the control that makes the ladder falsifiable: the eight
# LOWEST-ranked dials on their own. If the ranking is real, tail8 should do
# little; if tail8 performs like s23, the ranking is noise.
