...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14src
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14src.json" \
...
# crisis_blend_source, compensated so source * crisis_blend_gain stays at
# pt-v14's value, moves crisis co-movement in BOTH directions pt-v14 needs.
# On the two blocks measured so far:
#
#   source   mean    spread   lever   co-mov failures
#   1.00 v14 0.6745  0.0417   0.0151     1/2
#   0.95     0.6853  0.0379   0.0167     0/2
#   0.90     0.6960  0.0381   0.0152     0/2
#   0.85     0.7047  0.0339   0.0165     0/2
#   pt-v12   0.7043  0.0254   0.0227     0/2
#
# pt-v14 fails through the FLOOR because it sits 0.021 below the band
# centre. Lowering source raises the level back toward centre AND narrows
# the across-block spread, where every dial tried before traded one against
# the other. At source 0.90 the mean is 0.6960 against a band centre of
# 0.6955, and the panel and lever are unchanged.
#
# Two blocks is a direction, not a result. This is the walk from 0.86 to
# 0.98 on nine blocks none of them has seen, plus four neighbours each at
# plus or minus three percent on all seventeen dials for the two most
# promising settings -- because the lever's neighbourhood is exactly what
# keeps pt-v14 off the default, and any successor has to answer it too.
for BLOCK in "--seed-start 301" "--seed-start 501" "--seed-start 601" \
             "--seed-start 701" "--seed-start 801" "--seed-start 901" \
             "--seed-start 1001" "--seed-start 1201" "--seed-start 1401"; do
...
if [ -f /home/ec2-user/out/gates-301.json ] && [ -f /home/ec2-user/out/gates-1401.json ]; then
