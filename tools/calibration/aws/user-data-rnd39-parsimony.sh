...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd39-parsimony
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd39.json" \
...
# n19 clears five blocks of five -- the first vector in the programme to do
# it -- with the best 504-day panel and the best driven ratio, 1.366 against
# pt-v12's 1.551. And it carries THIRTY-THREE overrides, a third of the
# entire settable surface.
#
# That is not a preset. It is a recalibration wearing one.
#
# Twenty of those thirty-three came from round 36's screen, and a screen
# samples dials, it does not choose them. Of the twenty, exactly ONE had a
# strong effect: `market_vol_vix_anchor` at -0.97 against the driven ratio.
# The other nineteen move by between 0.6% and 18% with no individual
# justification, and n19 is one random draw of them. Nineteen coefficients
# nobody chose, carried because they happened to be in the winning sample,
# is exactly the thing this project's own history warns about -- pt-v12
# exists because someone found a constant nobody had chosen.
#
# So this is a parsimony test rather than another search:
#
#   c28              13 dials
#   c28 + anchor     14 dials, the one screened dial that mattered
#   c28 + half       14 dials, the anchor moved half as far
#   n19              33 dials
#
# If c28+anchor lands near n19, the other nineteen are decoration and the
# candidate to carry forward is fourteen dials rather than thirty-three. If
# it does not, then something among the nineteen matters and the screen has
