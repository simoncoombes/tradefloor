...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14j-sf94
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14j.json" \
...
# sf94 leads, and it is s23 with ONE dial moved: sector_factor_sigma from
# 0.0106173 to 0.0099803, a 6% reduction. On seven blocks it reads 6/7 on
# the 504-day panel against pt-v12's 1/7, ZERO crisis co-movement failures
# against pt-v12's 3, and a lever error of 0.0187 against 0.0293.
#
# That is the round 46 partial-effects prescription working exactly as
# written: sector_factor_sigma moves the co-movement LEVEL (-0.0117 per sd)
# while being spread-neutral (-0.0006) and lever-neutral (-0.0005), where
# every other lever this programme reached for moved two things at once.
# s23 had over-corrected the level to -0.0137 off centre; six percent off
# one dial brings it to -0.0043 and the failures stop.
#
# Notably it does NOT lower the persistence. The window finding predicted
# the winner would sit near 0.969; sf94 sits at s23's 0.9728 and wins by
# centring instead. Both runs are in flight and the prediction stands or
# falls on its own.
#
# This run: the five blocks sf94 has never seen, plus a one-dimensional
# walk of sector_factor_sigma either side of it (0.92 to 0.96 of s23's
# value), plus six neighbours of sf94 shaken +/-3% across all fifteen dials
# -- the era-boundary "region rather than a lucky point" test, run WITH
# confirmation rather than a round after it.
for BLOCK in "--seed-start 401" "--seed-start 901" "--seed-start 1001" \
             "--seed-start 1201" "--seed-start 1401"; do
