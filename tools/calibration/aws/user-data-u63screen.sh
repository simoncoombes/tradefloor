...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/u63screen
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-u63screen.json" \
...
# The untouched-dial screen: eighteen mechanisms the programme has never
# varied, one candidate each, on the pt-v15 base.
#
# Round 81 audited 732 measured vectors and found half the model's dials
# never moved; seven directions closed against pt-v14 and all seven came
# from the swept set. The mixture -- four dials from the untouched set --
# became pt-v15. This screens the rest: each candidate switches ONE
# mechanism on at the scale its own docstring names (the cascade's six
# components, the VIX response measured from FRED, the stress memory's
# weeks-scale decay), against a v15ship control.
#
# A SCREEN, not a qualification: four blocks rank directions for a
# thirteen-block confirmation, they do not certify anything. The blocks
# are chosen for power against the round-90 frontier: 201 is the lever's
# binding block, 401 the co-movement's, 1401 the co-movement block
# pt-v14 fails and pt-v15 takes, 801 is neutral.
#
...
for BLOCK in 201 401 801 1401; do
...
if [ -f /home/ec2-user/out/gates-1401.json ] && [ -f /home/ec2-user/out/gates-201.json ]; then
