...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/asym1
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-asym1.json" \
...
# asym1: three open questions, one run, on the folded six-override base.
#
# market_vol_gamma 0.02 / 0.04 and the alpha-for-gamma swap (0.24/0.08,
# same persistence, three times the asymmetry share): does the common
# factor's leverage effect move corr_asymmetry the +0.02-0.03 the full
# house needs, and what does it cost the vol and leverage rows?
#
# qe_pe_gain 0.0 / 0.5: round 94 measured the QE channel subtracting
# realism with either driven input; the dial is panel-visible through
# the endogenous central bank, so thirteen blocks judge it here.
#
# vm75: the volume row via a MEASURED cell on the folded base, after
# round 95 showed the assumed combination fails block 401 by 0.0013.
#
