...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd33-qualify-c05
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd33.json" \
...
# Round 31 screened seventeen dials this programme had never touched and
# found `crisis_vix_threshold` correlating -0.67 with the driven ratio while
# barely touching crisis co-movement. Round 32 held lh02's eight fixed and
# swept five of them. CONTROLLED, the threshold reads -0.71 against the
# driven ratio and +0.28 against co-movement -- stronger than the marginal
# estimate, and it HELPS the binding constraint rather than spending it,
# which nothing else in this programme does.
#
# c05 is lh02 plus five adjustments, none more than 6% from shipped. It has
# the same robustness margin as lh02, 0.163, the best of the round, with a
# better driven ratio on both screen blocks: 1.313 and 1.502 against 1.336
# and 1.535.
#
# c28 rides along as a PREDICTION rather than a candidate. Its driven ratio
# is better still, 1.292 and 1.465, and its margin is 0.044 against c05's
# 0.163. Round 29 established that vectors buying the objective with margin
# fail confirmation and round 25 watched four of them do it. If c28 clears
# the three fresh blocks and c05 does not, that rule is wrong and worth
# knowing.
#
# The bucketed driven run is the one that matters for the threshold.
# Raising it should move the VIX 30-45 bucket MORE than the others, because
# that is where crisis blending begins and where round 17 located the spike.
# If it lowers every bucket uniformly it is another level shift, and d3-f10
# already showed those leave the shape untouched.
export PRETIUM_S8_WORKERS=44
for spec in "v12:" \
            "c05:market_vol_alpha=0.300730582,market_vol_beta=0.66999041,jump_intensity_idio=0.0068895346,jump_sigma_idio=0.08369236,jump_intensity_market=0.0565753337,jump_vix_coupling=0.2626,idio_sigma_scale=0.5688,garch_vix_coupling=0.0269,crisis_vix_threshold=26.96740313,crisis_blend_gain=0.76402309,sector_factor_sigma=0.01132014,sector_loading=0.51498211,mispricing_half_life_days=63.01589113" \
            "lh02:market_vol_alpha=0.300730582,market_vol_beta=0.66999041,jump_intensity_idio=0.0068895346,jump_sigma_idio=0.08369236,jump_intensity_market=0.0565753337,jump_vix_coupling=0.2626,idio_sigma_scale=0.5688,garch_vix_coupling=0.0269"; do
  name="${spec%%:*}"; ov="${spec#*:}"
...
for spec in "v12:::" \
            ...
            "lh02:pt-v12:market_vol_alpha=0.300730582,market_vol_beta=0.66999041,jump_intensity_idio=0.0068895346,jump_sigma_idio=0.08369236,jump_intensity_market=0.0565753337,jump_vix_coupling=0.2626,idio_sigma_scale=0.5688,garch_vix_coupling=0.0269"; do
...
if [ -f /home/ec2-user/out/s8-v12.txt ] && [ -f /home/ec2-user/out/roster-c05.json ]; then
