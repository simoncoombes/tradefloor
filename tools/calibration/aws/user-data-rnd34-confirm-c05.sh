...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd34-confirm-c05
...
# c05 has been measured on its two SCREENING blocks and nowhere else.
# Round 33 qualifies it on section 8, the concentrated rosters and the
# bucketed driven window -- none of which is a fresh seed block. Round 27
# is the reason that gap matters: round 24's leaders cleared their screen
# and every one failed on the blocks they had not seen.
#
# So this is the missing half: the gate on train, 301 and 501, none of
# which either screen used.
#
# c28 is here as a written-down prediction. Its driven ratio is better than
# c05's -- 1.292 and 1.465 against 1.313 and 1.502 -- and its robustness
# margin is 0.044 against c05's 0.163. Round 29 established that vectors
# buying the objective with margin fail confirmation, and round 25 watched
# four of them do exactly that. The prediction is that c28 fails here and
# c05 holds. If it comes out the other way the rule is wrong.
