...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14c-confirm
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14c.json" \
...
# s23 clears the pt-v14 bar on the four blocks it was searched on: the
# 504-day panel goes 4/4 against pt-v12's 1/4, crisis co-movement is
# outside on 1 block against pt-v12's 2, and lever error is 0.0221
# against 0.0241. It regresses nothing, which is the whole bar and which
# neither r15-70 nor r15-86 could clear.
#
# Four blocks is not confirmation. A block's 504 panel score is 44% seed-
# block variance, so 4/4 on the SEARCH blocks is exactly the evidential
# shape that has fooled this programme repeatedly -- c28, p970 and n19 all
# looked clean on the blocks they were selected on.
#
# So: the nine blocks s23 has never seen. If it holds, pt-v14 is 13-block
# evidence rather than 4.
#
# Also carried: s22 (the other feasible vector), r15-70 and lh16 as
# reference candidates measured on identical blocks, and FOURTEEN
# NEIGHBOURS of s23 at +/-3% on each of its fifteen dials. The neighbours
# are the era-boundary checklist's "region rather than a lucky point" test,
# run at the same time as confirmation rather than a round later.
