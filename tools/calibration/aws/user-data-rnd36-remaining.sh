...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd36-remaining
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd36.json" \
...
# Round 31 screened seventeen dials nobody had touched and found
# `crisis_vix_threshold`, which produced the best candidate in the
# programme. THIRTY-FOUR live dials have still never been screened, and the
# same method is the obvious continuation.
#
# Twenty of them here, swept +/-20% around c28's thirteen, so the question
# is what ADDS to the best candidate rather than what beats pt-v12.
#
# Fourteen are excluded by evidence rather than by taste:
#   news_market_weight, news_sector_weight, news_peer_weight,
#   news_peer_weight_down, news_peer_vix_coupling -- section 85 measured
#     company_news contributing exactly zero day-cells in a bare run, so
#     these cannot move any certified statistic by construction
#   crisis_blend_ramp -- section 93 measured a tenfold change giving
#     bit-identical prices
#   price_hard_cap -- a guard at 50,000
#   crisis_blend_source, vix_return_source -- mode switches, not continuous
#   inflation_floor -- macro side, and the macro chain is separable to 1e-4
#
# What is in: the per-name GARCH bounds and cascade, the market jump's mean
# and sigma, the market variance floor and its VIX ANCHOR, the crisis blend
# cap, order flow, the size and spread exponents, the VIX up-gain and shock
# cap, and the volume process.
#
# `market_vol_vix_anchor` is the one to watch. It is the reference the
# variance target is measured against -- target scales with (VIX/anchor)^2 --
# so moving it rescales the entire volatility-versus-VIX curve rather than
# a point on it. Round 19 ruled out the ceiling and the coupling as the
# saturation cause and never tested the anchor.
