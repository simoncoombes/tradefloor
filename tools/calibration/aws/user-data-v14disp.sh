...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14disp
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14disp.json" \
...
# The cross-sectional dispersion mechanism, built in round 47 and never
# used in a candidate.
#
# Real rosters disperse in per-name volatility far more than this model
# does: over ten non-overlapping 252-day windows of the forty-name
# reference panel the interquartile ratio q3/q1 runs 1.273 to 1.486, where
# pt-v12 averages 1.205 and pt-v14 reaches 1.242 as a side effect of the
# sector change. idio_sigma_beta_exponent scales a name's idiosyncratic
# volatility by beta^k, and on pt-v12 alone it reached 1.290 at k=3.
#
# The question is whether it closes more of that gap ON pt-v14's vector
# without costing the 504 panel, crisis co-movement or the crisis lever.
# An inert dial's first use is a separate decision from the preset it rides
# on, and this run is that decision's evidence rather than an assumption
# that a dial built for a gap must be good for the model.
#
# The trimmed variants exist because raising per-name dispersion raises the
# pooled volatility too, and pt-v14 already sits inside that band with
# little room; idio_sigma_scale comes down to hold the level while the
# dispersion goes up.
#
# Four blocks spanning the co-movement range, so a dispersion change that
# quietly moves crisis correlation is caught rather than missed.
