...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/flowtrimbasket
...
WORKERS=24   # c8g.24xlarge alongside round 33
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/covid-roster-2020-2021.json" \
      /home/ec2-user/roster.json; then
  echo "ABORTING: cannot fetch the candidate list"
  shutdown -h now
  exit 1
fi
chown ec2-user:ec2-user /home/ec2-user/roster.json
...
# leveltrim: the level-tension campaign opens (rounds 98-101 lineage).
#
# The sim runs 20-25% hot at both held-VIX ends with the ratio perfect,
# and the driven window sees it directly. A market-factor sigma trim is
# scale-coherent: the lever is a ratio, correlations are scale-free, the
# panel vol has band room (31.55 in 16-34). Four trims against the full
# pt-v16 card on the screen blocks that bind everything: 201 (lever),
# 401 (co), 701/1201 (the fresh full-house wins). Driven is the prize
# row; the full card is the guard.
#
...
FLOW_GRID=trim python tools/calibration/flow_basket.py     --roster /home/ec2-user/roster.json     --workers WORKERS_PLACEHOLDER     --out /home/ec2-user/out/flow-trim-basket.json || true
aws s3 cp /home/ec2-user/out/flow-trim-basket.json "BUCKET_PLACEHOLDER/" || true
...
if [ -f /home/ec2-user/out/gates-201.json ] && [ -f /home/ec2-user/out/gates-201.json ]; then
