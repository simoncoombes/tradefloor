...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd45-distribution
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd44.json" \
...
# Round 44 established something about pt-v12 that nobody had: THE SHIPPED
# PRESET FAILS THE 504-DAY PANEL ON FOUR OF SEVEN THIRTY-SEED BLOCKS. Round
# 3 found the claim seed-dependent on five blocks; two more made the
# dependence larger, not smaller, and `envelope.py` states it flatly:
# "since pt-v12 all fourteen are ALSO in band at 504 days".
#
# Seven blocks is enough to know the claim is unreliable. It is not enough
# to say HOW unreliable, and the difference matters because the envelope has
# to say something specific. "Holds on three of seven" and "holds on nine of
# thirteen" are different sentences and only one of them can be written.
#
# So this measures the distribution rather than hunting another candidate.
# Six more untouched blocks -- 801, 901, 1001, 1101, 1201, 1301 -- taking
# pt-v12 to thirteen thirty-seed blocks, which is 390 seeds.
#
# r15-70 and d3-f10 ride along because the same question applies to them and
# because a candidate's failure rate is only meaningful beside the
# incumbent's. If pt-v12 holds at 504 on a third of blocks and r15-70 on two
# thirds, that is the honest way to state the improvement -- as a rate, not
# as a count on the blocks that happened to be run.
#
# This serves a decision rather than a search. The 504-day claim is
# published, and what replaces it should be a measured frequency.
for BLOCK in "--seed-start 801" "--seed-start 901" "--seed-start 1001" \
              "--seed-start 1101" "--seed-start 1201" "--seed-start 1301"; do
...
if [ -f /home/ec2-user/out/gates-801.json ] && [ -f /home/ec2-user/out/gates-1301.json ]; then
