...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd40-compensator
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-rnd40.json" \
...
# The parsimony test came back the OTHER way, and that is the useful part.
#
#   driven   c28 1.378 -> c28+anchor 1.328 -> n19 1.317
#   lever    c28 6.03/6.16 -> c28+anchor 5.68/5.83 -> n19 5.93/6.07
#
# The anchor buys 0.050 of n19's 0.061 driven improvement, so on that axis
# the other nineteen dials really are near-decoration. But the anchor COSTS
# 0.35 of crisis lever, and c28+anchor fails the lever tolerance on both
# blocks -- 7.8% and 5.4% from the real 6.16 against a 5% bar. n19 clears
# because something among the nineteen gives 0.25 of it back.
#
# So they are not decoration. They are a compensator that the screen found
# and nobody has identified. That is worse than decoration: nineteen
# unexplained numbers doing something real is exactly the state pt-v12's own
# hard-coded 4.0 was in before someone looked at it.
#
# This finds which one. c28+anchor is held fixed and the nineteen are swept
# between their shipped values and n19's, so the correlation with the crisis
# lever names the compensator directly. Round 36's screen correlated against
# the driven ratio and co-movement and never against the lever, which is why
# it went unseen.
#
# If one or two dials carry it, the candidate reduces from thirty-three to
# fifteen or sixteen with a reason attached to every one. If the effect is
# spread thinly across all nineteen, then n19 is genuinely irreducible and
# should be rejected on those grounds rather than shipped.
