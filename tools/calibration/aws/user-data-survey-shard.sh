...
SAMPLES=3000
# The crisis surface, sharded. Five axes over the space every gate batch
# since §92 has been hand-picking points in: the two couplings, the crisis
# gain that §97 unpinned, the idio scale that funds it, and the blend cap.
# One plan, SHARD_N boxes, each taking every SHARD_Nth vector by GLOBAL
# index so the tasks.jsonl files concatenate into exactly the whole run.
SHARD=SHARD_I/SHARD_N
SAMPLES=3000
BASE=pt-v10
ONLY=sector_vix_coupling,crisis_blend_gain,idio_sigma_scale,jump_vix_coupling,crisis_blend_cap
...
python tools/calibration/atlas_survey.py run --samples "$SAMPLES" --base "$BASE" --only "$ONLY" \
  --out /home/ec2-user/out --workers 94 --shard SHARD_PLACEHOLDER
# No collect here: this box holds one slice. The shards are concatenated and
# collected once, off the boxes, so the certificate covers the whole plan.
WORK

sed -i "s|BRANCH_PLACEHOLDER|${BRANCH}|" /home/ec2-user/run.sh
sed -i "s|BUCKET_PLACEHOLDER|${BUCKET}|" /home/ec2-user/run.sh
sed -i "s|\$SAMPLES|${SAMPLES}|g; s|\$BASE|${BASE}|g; s|\$ONLY|${ONLY}|g; s|SHARD_PLACEHOLDER|${SHARD}|" /home/ec2-user/run.sh
...
aws s3 cp /home/ec2-user/out/tasks.jsonl "$BUCKET/tasks.jsonl" || true
aws s3 cp /home/ec2-user/out/meta.json "$BUCKET/meta.json" || true
