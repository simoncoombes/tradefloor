...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/v14ctl
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-v14ctl.json" \
...
# THE CONTROL THAT SHOULD HAVE COME FIRST.
#
# pt-v14 is not the default because its crisis lever is a sharp optimum:
# six neighbours at +/-3% hold the panel and co-movement, only two keep the
# lever in tolerance. A one-at-a-time probe then localised it to three dials
# of fifteen -- market_vol_beta, market_vol_alpha and market_vol_vix_anchor,
# which break the 5% tolerance when moved alone where the other twelve stay
# under 0.038.
#
# Those three ARE the volatility-versus-VIX curve, and the crisis lever is a
# ratio of two points on that curve. So ANY vector should be sensitive
# there, and "pt-v14 is fragile" is not yet a claim -- it is an observation
# with nothing to compare against. pt-v12's neighbourhood has never been
# probed.
#
# A = pt-v12 perturbed. B = pt-v14 perturbed. Same three fragile dials, same
# four step sizes, plus three dials the probe found STABLE as a within-run
# control so a null result cannot be blamed on the method.
#
# If A and B degrade alike, the sensitivity is the model's and pt-v14 is no
# worse a default than the incumbent on this axis. If B degrades harder, the
# fragility is pt-v14's own and the search has to change.
for BLOCK in "--seed-start 201" "--seed-start 1101"; do
