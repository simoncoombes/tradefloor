...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/rnd2-funded
...
[
 ...
 {"label":"fund-0.50",    "base":"pt-v12","overrides":{"idio_sigma_scale":0.50}},
 ...
 {"label":"r25-f0.50",    "base":"pt-v12","overrides":{"inflation_reversion":0.25,"idio_sigma_scale":0.50}},
 {"label":"r25-f0.47",    "base":"pt-v12","overrides":{"inflation_reversion":0.25,"idio_sigma_scale":0.47}},
 {"label":"r15-f0.47",    "base":"pt-v12","overrides":{"inflation_reversion":0.15,"idio_sigma_scale":0.47}},
 {"label":"r15-f0.44",    "base":"pt-v12","overrides":{"inflation_reversion":0.15,"idio_sigma_scale":0.44}},
 {"label":"r15-c9-f0.44", "base":"pt-v12","overrides":{"inflation_reversion":0.15,"inflation_ceiling":9.0,"idio_sigma_scale":0.44}}
]
CANDS
...
python tools/calibration/gate_batch.py \
  ...
  --out /home/ec2-user/out/gates.json

# The gain, not only the cost. Round 1 priced inflation_reversion without
# establishing that it buys anything, which can only ever reject.
python tools/calibration/macro_range.py \
  --candidates /home/ec2-user/candidates.json \
  --seeds 30 --years 5 \
  --workers WORKERS_PLACEHOLDER \
  --out /home/ec2-user/out/macro.json
WORK
...
if [ -f /home/ec2-user/out/gates.json ] && [ -f /home/ec2-user/out/macro.json ]; then
  echo "CERTIFIED exit=$STATUS" > /tmp/DONE
else
  echo "FAILED exit=$STATUS no gates.json" > /tmp/DONE
fi

aws s3 cp /var/log/pretium-run.log "$BUCKET/run.log" || true
aws s3 cp /var/log/pretium-stream.log "$BUCKET/stream.log" || true
aws s3 cp /home/ec2-user/out/gates.json "$BUCKET/" || true
aws s3 cp /home/ec2-user/out/macro.json "$BUCKET/" || true
