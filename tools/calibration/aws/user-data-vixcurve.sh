...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/vixcurve
...
if ! aws s3 cp "s3://dia-test-101631415962-us-east-2-an/pretium-calib/in/cands-vixcurve.json" \
...
# vixcurve: the response exponent, lever-preserved (round 100).
#
# The square in the market variance target is too convex through mid-VIX
# along real paths -- the whole remaining driven excess. Four exponents
# (1.75, 1.5, 1.25, 1.0), each with the coupling re-fit so T(45)/T(5)
# holds, against the v16 base. One run answers driven, lever, co-movement
# and panel at once; the vix5 instrument is the stated risk (couplings
# above 1 send the target negative below VIX ~3.5, floor-clamped).
#
