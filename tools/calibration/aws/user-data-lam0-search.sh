...
BUCKET=s3://dia-test-101631415962-us-east-2-an/pretium-calib/out/lam0-search
...
v4 = pt.ModelParams.from_preset("pt-v5").to_dict()
vec = {k: v for k, v in v4.items() if k in settable}
pass  # pt-v5 already carries share 0.0
...
python tools/calibration/calibrate.py \
  ...
  --fix garch_alpha:0.059507211981547736 \
  --fix garch_beta:0.6853150814664563 \
  --fix garch_gamma:0.18318536187800277 \
  --fix idio_sigma_scale:0.8146007420925029 \
  --fix market_vol_alpha:0.46779624669755665 \
  --fix market_vol_beta:0.5212617214385166 \
  --fix market_vol_vix_coupling:0.9540498202999739 \
  --fix momentum_theta:0.07420624999999997 \
  --dual-horizon 504 \
  --lambda 0 \
  --workers 94 \
  --out /home/ec2-user/out/lam0-search.json
WORK
...
# its last line.
if [ -f /home/ec2-user/out/lam0-search.json ]; then
...
aws s3 cp /home/ec2-user/out/lam0-search.json "$BUCKET/" || true
