...
driven.py with notebook 09's path as of 2681878 (2026-09-26): AAPL on its
FY2019 accounts plus Universe.random(39, seed=2020), driven by the real VIX,
the FOMC midpoint, Moody's Baa as the corporate yield (FRED DBAA, the file's
baa column, percent) and the S&P-against-EMA-200 valuation proxy
(qe_pe_boost), with the NBER phases as the cycle, 505 days. driven.py drove
the HYG-converted high-yield proxy and no cycle. Everything else, including
the regressions and the event study, is driven.py's.
"""
...
def phases(d):
    """Notebook 09's phases, in the model's labels, from the NBER dates."""
    if d < dt.date(2020, 3, 1): return "peak"
    if d < dt.date(2020, 4, 1): return "contraction"
    if d < dt.date(2020, 5, 1): return "trough"
    if d < dt.date(2021, 5, 1): return "recovery"
    return "expansion"
...
def job(args):
    ...
    path = [{"day": i, "vix": raw["vix"][i], "federal_funds_rate": policy[i],
             ...
             "cycle": phases(dates[i])} for i in range(n)]
