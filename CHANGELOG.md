...
carries them. Both decide what the engine draws next, so two states alike
in every column but differing by one installed substitution are not the
same state and a commitment that skipped the table would say they were.
A ledger written before this change carries leaves computed without them
and does not verify against one written after it.
