...
//! stream's position.
//!
//! # D6, the session-band circuit breaker
//!
//! The +/-25% breaker clamps against `previousClose` during 09:30 to 16:00
//! and is applied TWICE, deliberately: once to the model price
//! (`fairValue * exp(s)`) and again to the settled print. The second clamp is
//! the one that matters, because a breaker that bounds an unobservable
//! reference and not the tape is not a breaker, and the gap between them is
//! where the measured 40.30% violations lived.
//!
