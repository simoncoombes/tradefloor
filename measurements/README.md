...
| `roster-shapes-pt-v19.json` | `tools/calibration/roster_shapes.py` on pt-v19: five sector mixes, seeds 101 to 130, 252 and 504 days, as collected from the design repository's fleet run `docs080b` (engine pin `ed15e73`, known-answer digest `1e683b96`) | `envelope.ROSTER_SHAPE_ROWS` and `ROSTER_INDEX_DRIFT`, which `check` reads for a sector-concentrated roster, and which `tests/test_envelope.py` re-scores from this file |
