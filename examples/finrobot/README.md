...
This is an agent-EVALUATION experiment, not a strategy-performance demo. One
seed cannot say which arm was the better investor. It can say, exactly, what
the same agent holding the same book did differently when one number changed
-- which is a question no historical dataset can answer, because history ran
once.
...
written out field by field, not a filter over the engine. Fair value, the
nine-way factor attribution of every price move, each company's mispricing and
the macro path the run has not reached yet all stay on the Tradefloor side of
the line. `tests/test_finrobot.py` proves it twice: once by running the
mapping against an engine that raises if the forbidden surface is touched, and
once by computing the hidden values and scanning the text FinRobot receives
for them.
...
market is deterministic; an LLM agent is not, and the two must not be allowed
to blur. It records the framework and version, the entry point, the provider,
...
This is a Tradefloor integration for FinRobot. It is not an official FinRobot
interface, and there is no affiliation with or endorsement by AI4Finance.

Everything reported here is ground truth about this simulated market. It is a
