...
replay takes a couple of seconds.
...
That calls the real thing. It makes sixty decisions, twenty days of shared
history plus twenty in each arm at one decision a day, so it costs sixty API
calls and takes a few minutes. Add `--record` to overwrite the replay fixture
with the run you just did.
...
The provenance file is what you cite a result from. tradefloor's market is
deterministic and an LLM agent is not, so the two must not blur together. It
records the framework and version, the entry point, the provider, the model,
the generation parameters, the mandate version and the decision cadence,
everything a replay cannot reconstruct from the market alone.
...
nor endorses it, and it is no part of FinRobot's own interface.
