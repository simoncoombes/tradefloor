...
UNIVERSE = tradefloor.Universe.random(8, seed=3)
...
def test_results_are_ordered_by_input_not_by_completion():
    ...
    """
    seeds = [11, 3, 7, 42, 5]
    swept = tradefloor.run_many(seeds, universe=UNIVERSE, ticks=80, workers=4)
    for i, seed in enumerate(seeds):
        alone = tradefloor.run_many([seed], universe=UNIVERSE, ticks=80)[0]
...
def test_worker_count_does_not_change_the_answer():
    ...
    # show at 8.
    seeds = list(range(10))
    one = tradefloor.run_many(seeds, universe=UNIVERSE, ticks=60, workers=1)
    for workers in (2, 4, 8):
        assert tradefloor.run_many(seeds, universe=UNIVERSE, ticks=60,
                                workers=workers) == one


def test_a_sweep_matches_running_each_seed_by_hand():
    seeds = [1, 2, 3]
    swept = tradefloor.run_many(seeds, universe=UNIVERSE, ticks=100, days=2, workers=2)
...
def test_different_seeds_give_different_markets():
    results = tradefloor.run_many(range(8), universe=UNIVERSE, ticks=60, workers=2)
...
def test_the_universe_crosses_as_a_specification():
    ...
    swept = tradefloor.run_many([4], universe=UNIVERSE, ticks=50)[0]
    rebuilt = tradefloor.Universe.from_json(UNIVERSE.to_json())
    direct = tradefloor.run_many([4], universe=rebuilt, ticks=50)[0]
    assert arr(swept) == arr(direct)


def test_collect_modes():
    summary = tradefloor.run_many([1, 2], universe=UNIVERSE, ticks=50,
    ...
    attribution = tradefloor.run_many([1], universe=UNIVERSE, ticks=50,
                                   collect="attribution")[0]
    assert sorted(attribution["columns"]) == sorted(tradefloor.Engine.FACTORS)
...
def test_a_macro_state_reaches_every_worker():
    ...
    swept = tradefloor.run_many([9], universe=UNIVERSE, ticks=80, macro=macro)[0]
    e = tradefloor.Engine(seed=9, universe=UNIVERSE, macro_state=macro)
    e.open_market()
...
def test_degenerate_inputs_are_refused():
    with pytest.raises(tradefloor.ValidationError, match="no seeds"):
        tradefloor.run_many([], universe=UNIVERSE)
    with pytest.raises(tradefloor.ValidationError, match="at least 1"):
        tradefloor.run_many([1], universe=UNIVERSE, ticks=0)
    with pytest.raises(tradefloor.ValidationError, match="unknown collect"):
        tradefloor.run_many([1], universe=UNIVERSE, ticks=10, collect="everything")
