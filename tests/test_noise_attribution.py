...
def test_arms_share_every_other_draw():
    ...
    # `market_vol_level` joined at 0.8.0 and `shock_scale` on 2026-09-16,
    # and both are blind for the reason the five before them are:
    # `draws_by_stream` counts the three streams an embedder can reach, and
    # a mechanism stream is not one of them.
    assert blind == {"jumps", "news", "volume", "volume_idio", "overnight",
                     "market_vol_level", "shock_scale"}
