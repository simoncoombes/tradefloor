...
def test_the_committed_recording_replays_end_to_end():
    ...
    # RE-RECORDED at the 0.8.0 boundary that made pt-v19 the default, the
    # second time this fixture has been remade for that reason. A recording
    # is keyed by a digest of the exact observation the model was sent, and
    # every price in that observation moved, so the committed transcript
    # could not replay at all and a fresh live run was the only way back.
    # These are that run. The values stay PINNED rather than bounded for the
    # reason the docstring gives -- a corrupted digest drops decisions
    # quietly and only exact numbers catch it.
    #
    # The three eras of this fixture, on the same seed, roster and brief:
    # pt-v16 3 trades, pt-v18 7, pt-v19 9. The market moved under the model
    # each time and the model answered it differently; none of the three is
    # a better agent than the others.
    assert card.trades == 9, card.trades
    assert card.pnl == pytest.approx(20520.0), card.pnl
    assert card.turnover == pytest.approx(2676010.0), card.turnover

    # AND THE REFUSAL IS BACK, twice, which is a fact about this market and
    # not a bug. gpt-5.2 sized inside the limits on pt-v18's market and the
    # pt-v18 recording had nothing to refuse; on pt-v19 it asked for 2.16x
    # and 2.09x against a 2.00x cap at the same step, and the MARKET refused
    # both legs. That is the environment doing its job on a decision the
    # agent made, not a replay failure.
    #
    ...
    assert len(card.errors) == 2 and all(
