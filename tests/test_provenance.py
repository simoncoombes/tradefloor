...
DECOY = tradefloor.Universe.random(20, seed=99)
...
def test_a_scorecard_names_its_market():
    scores = tradefloor.evaluate({"hold": BuyAndHold()}, seed=2026,
...
def test_facts_name_their_market():
    facts = tradefloor.facts.measure(seed=3, universe=UNIVERSE, days=60)
    assert facts["universe_fingerprint"] == UNIVERSE.fingerprint
    assert facts["seed"] == 3


def test_an_execution_names_its_market():
    execution = tradefloor.tca.analyse(Buyer(), seed=2026, universe=UNIVERSE,
                                    days=1)
...
def test_the_three_agree_on_one_universe():
    ...
    scores = tradefloor.evaluate({"hold": BuyAndHold()}, seed=1,
                              universe=UNIVERSE, days=2)
    facts = tradefloor.facts.measure(seed=1, universe=UNIVERSE, days=60)
    execution = tradefloor.tca.analyse(Buyer(), seed=1, universe=UNIVERSE, days=1)
    assert {
...
@pytest.mark.parametrize("produce", [
    lambda u: tradefloor.evaluate({"hold": BuyAndHold()}, seed=2026, universe=u,
                               days=2)["hold"].universe_fingerprint,
    lambda u: tradefloor.facts.measure(seed=3, universe=u, days=60)[
        "universe_fingerprint"],
    lambda u: tradefloor.tca.analyse(Buyer(), seed=2026, universe=u,
...
def test_a_plain_list_is_fingerprinted_the_same_as_a_universe():
    ...
    plain = list(UNIVERSE)
    facts = tradefloor.facts.measure(seed=3, universe=plain, days=60)
    assert facts["universe_fingerprint"] == UNIVERSE.fingerprint
...
def test_a_sweep_row_names_its_market():
    ...
    """
    rows = tradefloor.run_many(seeds=[1, 2, 3], universe=UNIVERSE, days=1,
...
def test_sweep_rows_distinguish_the_decoy_where_tickers_do_not():
    ours = tradefloor.run_many(seeds=[1], universe=UNIVERSE, days=1, ticks=40,
                            collect="summary")[0]
    theirs = tradefloor.run_many(seeds=[1], universe=DECOY, days=1, ticks=40,
                              collect="summary")[0]
...
def test_the_stamp_survives_the_worker_boundary():
    ...
    # serial paths agree.
    serial = tradefloor.run_many(seeds=[1, 2, 3, 4], universe=UNIVERSE, days=1,
                              ticks=40, workers=1, collect="summary")
    threaded = tradefloor.run_many(seeds=[1, 2, 3, 4], universe=UNIVERSE, days=1,
