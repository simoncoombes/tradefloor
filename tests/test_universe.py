...
def test_the_same_seed_gives_a_citable_universe():
    ...
    a = pretium.Universe.random(108, seed=7)
    b = pretium.Universe.random(108, seed=7)
...
def test_a_different_universe_seed_gives_a_different_universe():
    a = pretium.Universe.random(50, seed=1)
    b = pretium.Universe.random(50, seed=2)
...
def test_the_universe_seed_is_independent_of_the_simulation_seed():
    ...
    a lie.
    """
    universe = pretium.Universe.random(8, seed=99)
    a = pretium.Engine(seed=1, universe=universe)
    b = pretium.Engine(seed=2, universe=universe)
    for e in (a, b):
        e.open_market()
        e.run_session(9, 30, 3, 40)
    assert a.prices() != b.prices(), "different sim seeds must differ"

    c = pretium.Engine(seed=1, universe=pretium.Universe.random(8, seed=99))
    c.open_market()
...
def test_generation_rejects_degenerate_sizes():
    with pytest.raises(pretium.ValidationError):
        pretium.Universe.random(0)
    with pytest.raises(pretium.ValidationError, match="three letters"):
        pretium.Universe.random(26 ** 3 + 1)
...
def test_a_universe_is_an_ordered_sequence():
    ...
    u = pretium.Universe.random(5, seed=1)
...
def test_json_round_trips_exactly():
    u = pretium.Universe.random(40, seed=11)
    back = pretium.Universe.from_json(u.to_json())
...
def test_a_serialised_universe_produces_the_same_market():
    ...
    u = pretium.Universe.random(6, seed=4)
    a = pretium.Engine(seed=3, universe=u)
    b = pretium.Engine(seed=3, universe=pretium.Universe.from_json(u.to_json()))
    for e in (a, b):
        e.open_market()
...
def test_a_newer_schema_is_refused_rather_than_partially_read():
    ...
    doc = json.loads(pretium.Universe.random(2, seed=1).to_json())
    doc["schema"] = 99
    with pytest.raises(pretium.ValidationError, match="newer"):
        pretium.Universe.from_json(json.dumps(doc))


def test_unknown_fields_are_refused():
    doc = json.loads(pretium.Universe.random(2, seed=1).to_json())
    doc["instruments"][0]["mystery"] = 1
    with pytest.raises(pretium.ValidationError, match="mystery"):
        pretium.Universe.from_json(json.dumps(doc))
...
def test_a_missing_required_field_names_itself():
    ...
    with pytest.raises(pretium.ValidationError, match="ticker"):
        pretium.Universe.from_json(json.dumps(doc))


def test_nonsense_input_is_refused():
    with pytest.raises(pretium.ValidationError, match="not a pretium universe"):
        pretium.Universe.from_json('{"hello": 1}')
...
def test_a_scenario_is_a_path_not_a_flag():
    ...
    disguise.
    """
    e = pretium.Engine(seed=1, universe=pretium.Universe.random(6, seed=1),
                       macro_state=pretium.Macro(federal_funds_rate=0.025))
...
def test_macro_reads_back_in_the_units_it_was_written_in():
    ...
    e = pretium.Engine(seed=1, universe=pretium.Universe.random(3, seed=1),
                       macro_state=pretium.Macro(federal_funds_rate=0.0425,
...
def test_a_driven_path_is_reproducible():
    def run():
        e = pretium.Engine(seed=8, universe=pretium.Universe.random(5, seed=2))
