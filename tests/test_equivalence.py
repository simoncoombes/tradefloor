...
def test_run_days_matches_a_hand_written_loop():
    packed = pretium.Engine(seed=5, universe=UNIVERSE)
    packed.run_days(3, record=False)

    manual = pretium.Engine(seed=5, universe=UNIVERSE)
    for _ in range(3):
        manual.open_market()
...
def test_run_session_matches_a_loop_of_ticks():
    ...
    chunked = pretium.Engine(seed=5, universe=UNIVERSE)
    chunked.open_market()
    chunked.run_session(9, 30, 3, 5)

    stepped = pretium.Engine(seed=5, universe=UNIVERSE)
    stepped.open_market()
    hour, minute = 9, 30
    for _ in range(5):
...
def test_run_until_matches_an_unclosed_session():
    ...
    open_ended = pretium.Engine(seed=5, universe=UNIVERSE)
...
def test_a_serialised_universe_is_the_same_universe():
    ...
    a = pretium.Engine(seed=9, universe=UNIVERSE)
    b = pretium.Engine(seed=9, universe=rebuilt)
