...
@pytest.mark.xfail(
    reason=(
        "KNOWN REGRESSION at the pt-v3 era boundary, left red on purpose and "
        "much reduced since. On this fixture the shock/flat ratio at seeds "
        "3, 5, 7 reads x1.064, x1.062, x1.037 against pt-v1's x1.23, x1.25, "
        "x1.17 -- two of three now clear the bar and the third is marginal. "
        "The first pt-v3 candidate read x1.022 at seed 3, so most of the "
        "loss is already recovered.\n\n"
        "Cause, measured across three independently searched vectors: the "
        "market factor's variance reverts to a target scaling with "
        "(VIX/15)^2, and how fast it gets there is market_vol_alpha + "
        "market_vol_beta. pt-v1 reverts over 13.5 days and a 20-day spike "
        "moves it; the first pt-v3 candidate reverted over 192 days and the "
        "spike was over before the variance noticed. The shipped vector caps "
        "the half-life at 63 days, which is why most of the lever came back "
        "-- held-VIX lever x2.03 against pt-v1's x2.51, 80.7% retained, "
        "against 61.1% for the uncapped candidate.\n\n"
        "It is not fixed, and the reason it cannot be fixed by search is the "
        "finding: ONE variance timescale is doing two jobs. Within-year "
        "clustering wants long memory and VIX response wants short, so every "
        "vector is a compromise on a line -- and the 504-day horizon axis "
        "reads 1.81 at 63 days against 0.0058 at 192. The repair is a "
        "multi-timescale variance process, where a fast component tracks VIX "
        "and a slow one carries clustering. That would also retire the decay "
        "shape defect (clustering gone by lag 20 where real markets persist "
        "to 60), which has the same single-exponential cause.\n\n"
        "Tracked by tools/calibration/scenario_response.py, which exists "
        "because nothing in the calibration noticed this: every guard reads "
        "the realism panel, and the panel is measured at one flat VIX."
    ),
    strict=True,
)
